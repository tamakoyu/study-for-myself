...
**Obsidian 存数据 + 本地程序管复习**。错题只是其中一块 ——
今日 / 错题 / 计划 / 笔记 / 复盘，五个页面都直接读写你 Obsidian 里的 Markdown。
...
- **笔记**：左侧目录树（收 `config.json` 里 `noteDirs` 指定的目录），右侧渲染正文，公式用 KaTeX。
- **复盘**：按日期自动归档到 `复盘/26.9/第二周/9.13复盘.md`（周次取自对应周计划），
  直接编辑保存，Obsidian 里立刻能看到。
...
├── test/e2e.mjs            无头浏览器端到端测试（68 项断言）
├── backups/                自动备份（不入库）
└── data/                   导出（不入库）
```
...
覆盖：**今日倒计时/今日任务/勾选写回计划**、**计划页分组与复选框**、**笔记树与渲染**、
**复盘自动归档与保存**、四层总览下钻、顶栏范围选择器、题库筛选、抽屉预览、**全屏做题模式**、
